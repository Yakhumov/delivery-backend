...
if (-not (Test-Path -LiteralPath $backupPath)) {
  Copy-Item -LiteralPath $seedFullPath -Destination $backupPath
}

$pattern = '\{[^\r\n{}]*id:\s*(?<id>\d+),\s*name:\s*"(?<name>[^"]+)",[^\r\n{}]*?price:\s*[^,}]+,[^\r\n{}]*?categoryId:\s*\d+,[^\r\n{}]*?imageUrl:\s*"(?<url>https?://[^"]+)"[^\r\n{}]*\}'
$matches = [regex]::Matches($seed, $pattern)
...
foreach ($entry in $replacements) {
  $updatedSeed = $updatedSeed.Replace($entry.Original, $entry.Updated)
}

Set-Content -LiteralPath $seedFullPath -Value $updatedSeed -Encoding UTF8
Write-Host "Updated $($replacements.Count) image URLs."
